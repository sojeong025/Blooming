...
import { dressState } from "../../recoil/ProductAtom";
...
export default function WeddingHall() {
  ...
  const [dress, setDress] = useRecoilState(dressState)
  ...
  const fetchData = async () => {
    try {
      const response = await customAxios.get("product/DRESS", { params: {page: currentPage, size: 4} });
      
      if (response.data.result[0].last) {
        setHasMore(false);
      } else {
        setDress((prevProducts) => [...prevProducts, ...response.data.result[0].content]);

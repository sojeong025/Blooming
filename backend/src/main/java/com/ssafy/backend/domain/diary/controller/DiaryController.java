...
public class DiaryController {
    ...
    @Parameter(name = "userId", description = "회원 id로 불러옵니다.")
    @GetMapping("/diary")
    ...
    public ResponseEntity<BasicResponse> getOneDiary (@PathVariable Long diaryId) {
        DiaryResultDto diaryResultDto = diaryService.getOneDiary(diaryId);
        BasicResponse basicResponse;
        if (diaryResultDto == null) {
            basicResponse = BasicResponse.builder()
                    .code(HttpStatus.NO_CONTENT.value())
                    .httpStatus(HttpStatus.NO_CONTENT)
                    .message("다이어리 상세 조회 실패")
                    .count(0).build();
        } else {
            basicResponse = BasicResponse.builder()
                    .code(HttpStatus.OK.value())
